...
public class CreateOrderController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        ObservableList<String> parks =
                FXCollections.observableArrayList("Carmel", "Banias", "Hula");
        parkComboBox.setItems(parks);

        ObservableList<String> times =
                FXCollections.observableArrayList("09:00","10:00","11:00","12:00","13:00","14:00","15:00","16:00");
        timeComboBox.setItems(times);

        paymentComboBox.setItems(
                FXCollections.observableArrayList("Credit Card", "Cash")
        );

        int maxVisitors = 100;

        if (GoNatureClient.currentVisitor != null &&
                "Guide".equals(GoNatureClient.currentVisitor.getVisitorType())) {
            maxVisitors = 16;
        }

        visitorsSpinner.setValueFactory(
                new SpinnerValueFactory.IntegerSpinnerValueFactory(1, maxVisitors, 1)
        );
    }
    ...
    void submitOrder(ActionEvent event) {
        ...
        if (selectedPark == null || selectedDate == null || selectedTime == null ||
                paymentMethod == null || visitorsAmount.trim().isEmpty()) {
        ...
        boolean subscriber =
                GoNatureClient.currentVisitor.getSubNumber() != 0 && !visitorType.equals("Guide");

        String visitType =
                "Guide".equals(visitorType) ? "GUIDE_PREORDER" : "REGULAR_PREORDER";

        boolean prepaid =
                "Guide".equals(visitorType) && "Credit Card".equals(paymentMethod);
    ...
    void goBack(ActionEvent event) {
        if (GoNatureClient.currentVisitor != null) {
            try {
                ClientUI.send(new Message("FETCH_VISITOR_ORDERS",
