...
/**
 * The primary client communication class for the GoNature application.
 * Extends the OCSF AbstractClient to handle network connections, sending 
 * requests to the server, and receiving server responses via the Strategy Pattern.
 */
public class GoNatureClient extends AbstractClient {
	 ...
	 * Extracts the command from the message and uses the StrategyFactory 
	 * to execute the appropriate behavior.
	 ...
	 * thread that is waiting for messages from the server. This indicates 
	 * a dropped or interrupted connection.
	 * * @param exception The exception that caused the connection to drop.
	 */
	 ...
	 * Sends an initial "CONNECT" message to the server containing the client's PC name.
	 */
	@Override
	 ...
	 * Sends a request to the server to update a specific order's visit date and visitor count.
	...
	/**===============================DONT THINK THIS IS USED
	 * Sends a request to the server to cancel a specific order.
	...
	/**===============================DONT THINK THIS IS USED
	 * Sends a request to the server to add a new order to the waiting list.
	 * * @param orderData An ArrayList containing the details of the order to be waitlisted.
	 */
