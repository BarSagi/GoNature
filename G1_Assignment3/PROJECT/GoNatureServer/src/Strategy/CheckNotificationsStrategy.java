...
/**
 * Strategy class responsible for checking unread notifications for a user. The
 * strategy receives the user's email, gets unread notifications from the
 * database, marks them as read, and sends them back to the client.
 */
public class CheckNotificationsStrategy implements MessageStrategy {

	/**
	 * Executes the check notifications command. The method gets unread
	 * notifications by email, marks them as read if needed, and sends the
	 * notification list back to the client.
	 *
